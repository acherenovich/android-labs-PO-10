...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        geoPointIcon = ContextCompat.getDrawable(this, R.drawable.point2);
    ...
    private void stopTrackingService() {
        // Проверяем, был ли сервис запущен
        ...
        if (serviceIntent != null ) {
            ...
            Toast.makeText(this, "Сервис отслеживания остановлен", Toast.LENGTH_SHORT).show();
        }
    ...
    private void drawPolyline(List<GeoPoint> points) {
        if (points.size() < 2) return; // Линия должна содержать хотя бы 2 точки

        Log.d("LOADSAVEDLOCATIONS", "DrawPolyline");

        polyline = new Polyline();
        polyline.setPoints(points);
        //polyline.setColor(Color.BLUE); // Цвет линии
        //polyline.setWidth(5.0f); // Толщина линии

        mapView.getOverlayManager().add(polyline); // Добавляем линию на карту
...
//        List<GeoPoint> savedPoints = db.getLocations();
//        if (!savedPoints.isEmpty()) {
//            //trackPoints.addAll(savedPoints);
//            for (GeoPoint point : savedPoints) {
//                addLocationToMap(point.getLatitude(), point.getLongitude());
//            }
//        }
//    }
...
//
//            if (!savedPoints.isEmpty()) {
//                // Рисуем только первую и последнюю точку как маркеры
//                addLocationToMap(savedPoints.get(0).getLatitude(), savedPoints.get(0).getLongitude());
//                addLocationToMap(savedPoints.get(savedPoints.size() - 1).getLatitude(), savedPoints.get(savedPoints.size() - 1).getLongitude());
//            }
//        }
//    }
    ...
    private void loadSavedLocations() {
        ...
        for (Track track : tracks) {
            ...
            if (!savedPoints.isEmpty()) {
                ...
                // Устанавливаем центр карты на первую точку, если это первая загруженная трасса
                if (!firstPointSet) {
                    mapView.getController().setCenter(savedPoints.get(0));
                    firstPointSet = true;
                }

                // Добавляем маркеры в начале и в конце пути
                //addLocationToMap(savedPoints.get(0).getLatitude(), savedPoints.get(0).getLongitude());
                //addLocationToMap(savedPoints.get(savedPoints.size() - 1).getLatitude(), savedPoints.get(savedPoints.size() - 1).getLongitude());
            }
        }
