...
public class GestureActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        saveButton.setOnClickListener(view -> {
            ...
            gestureLibrary.removeEntry(name); // Удаляем старые жесты с таким именем
            gestureLibrary.addGesture(name, lastGesture);
            gestureLibrary.save(); // Сохраняем библиотеку

            // Полностью перезагружаем библиотеку жестов
            gestureLibrary = GestureLibraries.fromFile(new File(getFilesDir(), "gestures"));
            gestureLibrary.load();

            lastGesture = null; // Сбрасываем последний жест
            Toast.makeText(this, "Жест '" + name + "' сохранён и доступен сразу!", Toast.LENGTH_SHORT).show();
        });
