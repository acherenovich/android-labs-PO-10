...
public class MainActivity extends AppCompatActivity {
        ...
        gestureOverlay.addOnGesturePerformedListener((overlay, gesture) -> {
            try {
                ...
                if (predictions == null || predictions.isEmpty()) {
                    throw new Exception("Нет предсказаний жеста");
                }
                ...
                if (bestPrediction.score > 2.0) {
                    ...
                    throw new Exception("Жест имеет слишком низкий балл");
                }
            } catch (Exception e) {
    ...
    private void toggleMode(Button toggleModeButton) {
        ...
        if (isCalculatorMode) {
            display.setHint("Введите жест для калькулятора...");
            toggleModeButton.setText("Режим: Калькулятор");
        } else {
            display.setHint("Введите жест для отображения...");
            toggleModeButton.setText("Режим: Жесты");
        }
        clearDisplay();
    }
    ...
    private void handleGesture(String gestureName) {
        if (isCalculatorMode) {
            ...
            display.setText("Жест: " + gestureName);
        }
    }
