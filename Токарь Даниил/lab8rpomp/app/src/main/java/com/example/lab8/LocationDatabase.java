...
public class LocationDatabase extends SQLiteOpenHelper {
    ...
    public void deleteAllTracksAndLocations() {
        SQLiteDatabase db = this.getWritableDatabase();
        // Сначала удаляем данные из таблицы locations, связанные с маршрутами (опционально, если CASCADE DELETE не настроено)
        db.delete(TABLE_LOCATIONS, COLUMN_TRACK_FK + " IN (SELECT " + COLUMN_TRACK_ID + " FROM " + TABLE_TRACKS + ")", null);
        // Затем удаляем все маршруты из таблицы tracks
        db.delete(TABLE_TRACKS, null, null);
        db.close();
    }

    // Метод для удаления одного маршрута по ID (более гибкий вариант)
    public void deleteTrack(long trackId) {
        SQLiteDatabase db = this.getWritableDatabase();
        // Сначала удаляем все locations, связанные с trackId
        db.delete(TABLE_LOCATIONS, COLUMN_TRACK_FK + " = ?", new String[]{String.valueOf(trackId)});
        // Затем удаляем сам track
        db.delete(TABLE_TRACKS, COLUMN_TRACK_ID + " = ?", new String[]{String.valueOf(trackId)});
        db.close();
    }
